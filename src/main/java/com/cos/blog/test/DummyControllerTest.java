...
public class DummyControllerTest {
	...
	@Transactional //함수 종료시에 자동 커밋이 됨. save를 사용하지 않아도 update가 된다.
	@PutMapping("/dummy/user/{id}")
	public User updateUser(@PathVariable int id, @RequestBody User requestUser) { //json 데이터를 요청 => Java Object(MessageConverter의 Jackson라이브러리가)로 변환해서 받아줘요.
	...
	public User detail(@PathVariable int id) {
		...
		//그럼 return null 이 리턴이 되자나, 그럼 프로그램에 문제가 있지 않겠냐?
...
//		});
		...
		//스프링부트 = MessageConverter란 애가 응답시에 자동 작동
		//만약에 자바 오브젝트를 리턴하게 되면 MessageConverter가 Jackson 라이브러리를 호출해서
		//user 오브젝트를 json으로 변환해서 브라우져에게 던져준다.
		return user;
	}
